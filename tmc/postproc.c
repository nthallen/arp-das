...
#include "tmcstr.h"
#include "tmc.h"
#include "dbr.h" /* defines tm_info_type */
...
void post_processing(void) {
  ...
  { /* output dbr_info (after skeleton to assure dbr.h included */
	...
	if ( verfd < 0 ) {
	  ...
	  strcpy( tmi.tmid.ident, "1.0" );
	} else {
	  int i;
	  read( verfd, tmi.tmid.ident, 5 );
	  close( verfd );
      for ( i = 0; i < 5; i++ ) {
        if ( tmi.tmid.ident[i] == '\n' || tmi.tmid.ident[i] == '\r' )
          break;
      }
      while ( i < 5 ) tmi.tmid.ident[i++] = '\0';
	}
	memcpy( &tmi.tmid.ident[5], md5_sig, 16 );
	...
	tmi.isflag = SynchInverted ? ISF_INVERTED : 0;
	...
	{ int i;
	  for ( i = 0; i < 21; i += 7 ) {
		int j;
		for (j = 0; j < 7; j++ ) {
		  fprintf( ofile, " %3d,", tmi.tmid.ident[i+j] );
		}
		fprintf( ofile, "\n " );
	  }
	}
