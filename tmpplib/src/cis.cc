/* cis.cc Defines functions used by Command Interpreter Server */
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
#include <stddef.h>
...
extern "C" {
  ...
  static int io_write(resmgr_context_t *ctp, io_write_t *msg,
    RESMGR_OCB_T *ocb);
  static int io_notify(resmgr_context_t *ctp, io_notify_t *msg,
    RESMGR_OCB_T *ocb);
  static int io_close_ocb(resmgr_context_t *ctp, void *rsvd,
    RESMGR_OCB_T *ocb);
...
static struct ocb *ocb_calloc (resmgr_context_t *ctp, IOFUNC_ATTR_T *device) {
  ocb_t *ocb = (ocb_t *) calloc( 1, sizeof(ocb_t) );
  if ( ocb == 0 ) return 0;
...
static IOFUNC_ATTR_T *cis_setup_rdr( const char *node ) {
  ioattr_t *rd_attr = (ioattr_t *)new_memory(sizeof(ioattr_t));
  char nodebase[80], *nodename;
  int id;
  rdrs_t *rs = (rdrs_t *)new_memory(sizeof(rdrs_t));
...
cmdif_rd::cmdif_rd(const char *name_in) {
  name = name_in;
  handle = 0;
}
...
cmdif_wr::cmdif_wr(const char *name_in, const char *path_in) {
  name = name_in;
  path = path_in;
  ofd = -1;
}
...
void cmdif_wr::Setup() {
  ofd = open(tm_dev_name(path), O_WRONLY);
  if (ofd == -1)
    nl_error( 2, "Unable to open %s", path );
}

void cmdif_wr::Turf(const char *format, ...) {
  va_list arglist;
  char cmd[CMD_MAX_COMMAND_OUT+1];
  int nb, nbw;

  if ( ofd == -1 ) Setup();
  if ( ofd == -1 ) return;
  va_start( arglist, format );
  nb = vsnprintf( cmd, CMD_MAX_COMMAND_OUT, format, arglist );
  va_end( arglist );
  if ( nb >= CMD_MAX_COMMAND_OUT ) {
    nl_error( 2, "Output buffer overflow to interface %s", name );
  } else {
    nbw = write( ofd, cmd, nb );
    if ( nbw == -1 ) {
      nl_error( 2, "Error %d writing to interface %s", errno, name );
      close( ofd );
      ofd = -1;
    } else if (nbw != nb) {
      nl_error( 2, "Write to interface %s returned %d, expected %d",
	name, nbw, nb );
    } else if (nbw == 0) {
      close(ofd);
      ofd = -1;
...
void cmdif_dgdata::Turf() {
  if ( id == 0 )
    id = Col_send_init( name, data, dsize, 0);
  Col_send(id);
}
