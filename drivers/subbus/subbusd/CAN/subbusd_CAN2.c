...
/** The request queue */
static can_request_t sbdrq[SUBBUSD_MAX_REQUESTS];
static can_request_t *cur_req;
static unsigned int sbdrq_head = 0, sbdrq_tail = 0;
/** cur_rep is reserved for use of cur_req. Any other reply (e.g.
 * a syntax error in an incoming request) must use another structure.
 * This is to allow multi-read operations early access to the reply
 * structure.
 */
subbusd_rep_t cur_rep;
/** The number of bytes of data copied into cur_rep */
uint16_t rep_recd;
uint8_t rep_seq_no;
uint8_t req_no;
struct can_frame reqfrm;
/** True if we expect a response based on the request already transmitted */
...
enum { st_init, st_init_retry, st_operate } slcan_state = st_init;
...
void process_requests(void) {
  ...
  while ( sbdrq_head != sbdrq_tail && cur_req == NULL ) {
    ...
    nl_assert( sbr->status == SBDR_STATUS_ENQUEUED );

    while (!request_pending) {
      req_seq_no = sbr->sb_can_seq;
      offset = req_seq_no ? (req_seq_no*7 - 1) : 0;
      nl_assert(offset < sbr->sb_nc);
      nbdata = sbr->sb_nc - offset;
      reqfrm.can_id = CAN_REQUEST_ID(sbr->device_id,req_no);
      if (sbr->sb_can_seq) {
        if (nbdata > 7) nbdata = 7;
        reqfrm.can_dlc = nbdata+1;
        reqfrm.data[0] = CAN_CMD(sbr->sb_can_cmd,sbr->sb_can_seq);
        memcpy(&reqfrm.data[1], &sbr->sb_can[offset], nbdata);
      } else {
        rep_recd = 0;
        if (nbdata > 6) nbdata = 6;
        reqfrm.can_dlc = nbdata+2;
        reqfrm.data[0] = CAN_CMD(sbr->sb_can_cmd,req_seq_no);
        reqfrm.data[1] = sbr->sb_nc;
        memcpy(&reqfrm.data[2], &sbr->sb_can[offset], nbdata);
      }
      ++sbr->sb_can_seq;
      if (offset+nbdata >= sbr->sb_nc) {
  ...
  //########
      ...
      // ### send_packet() must verify that all output is written
      // if (!iface->obuf_clear()) {
      ...
      // }
      if (request_pending) {
        msg(MSG_DBG(1), "Setting timeout");
        set_timeout(0,50);
        // iface->TO.Set(0,50);
        // iface->flags |= DAS_IO::Interface::Fl_Timeout;
      } else {
        msg(MSG_DBG(1), "Request resolved immediately");
      }
  //########
...
/**
 * Prepares the first/next subbus_CAN command
 ...
 */
void process_mread(can_request_t *sbr) {
    ...
    case ':':
      ...
      if (arg2 == 1) {
        ...
        for ( ; arg1 <= arg3; arg1 += arg2) {
          can_msg.sb_can[can_msg.sb_nc] = arg1 & 0xFF;
          ++can_msg.sb_nc;
        }
        format_mread_rd();
        break;
...
can_request_t *new_request(int rcvid) {
  can_request_t *sbr = &sbdrq[sbdrq_tail];
  ...
  sbr->rcvid = rcvid;
  sbr->status = SBDR_STATUS_ALLOCATED;
  sbdrq_tail = new_tail;
}

/**
 * setup a basic single read/write request.
 */
void setup_rw_request(int rcvid, uint8_t can_cmd, uint16_t addr, uint16_t data) {
  can_request_t *sbr = new_request(rcvid);
  sbr->sb_can_cmd = can_cmd;
  sbr->device_id = (addr>>8) & 0xFF;
  sbr->sb_can_seq = 0;
  sbr->mr_nc = sbr->mr_cp = 0;
  sbr->sb_cp = 0;
  sbr->sb_can[0] = addr & 0xFF;
  switch (can_cmd) {
    case CAN_CMD_CODE_RD:
      sbr->sb_nc = 1;
      break;
    case CAN_CMD_CODE_WR_INC:
      sbr->sb_nc = 3;
      sbr->sb_can[1] = data & 0xFF;
      sbr->sb_can[2] = (data>>8) & 0xFF;
      break;
    default:
      msg(4, "Invalid sb_can_cmd %u in setup_request()", can_cmd);
  }
  sbr->status = SBDR_STATUS_ENQUEUED;
  if (sbdrq[sbdrq_head].stat == SBDR_STATUS_ENQUEUED) {
    process_request();
  }
}
 ...
 * setup a basic single read/write request.
 */
void setup_mread_request(int rcvid, char *mrcmd, uint16_t n_reads) {
  int i;
  can_request_t *sbr;
  subbusd_rep_t rep;
  char *s;
  uint16_t n_reads_req;
  int16_t status = SBS_OK;

  s = mrcmd;
  if (*s != 'M') {
    status = SBS_REQ_SYNTAX;
    nl_error(2, "Missing 'M' at start of multi-read request");
  } else {
    ++s;
    if (!read_hex(&s, &n_reads_req) || *s != '#' || n_reads_req != n_reads) {
      status = SBS_REQ_SYNTAX;
      nl_error(2, "Missing or invalid count or # after 'M' in multi-read request");
    }
  }
  ...
  if ( i >= SB_CAN_MAX_REQUEST ) {
    status = SBS_REQ_SYNTAX;
    nl_error(2, "Request exceeds %d characters", SB_CAN_MAX_REQUEST-1 );
  }
  if (status != SBS_OK) {
  ...
  sbr = new_request(rcvid);
  strncpy(sbr->mread, s, i);
  sbr->mread[i] = '\0';
  sbr->mr_nc = i;
  sbr->mr_cp = 0;
  sbr->sb_can_cmd = 7; // Invalid value for now
  sbr->sb_can_seq = 0;
  sbr->status = SBDR_STATUS_ENQUEUED;
  if (sbdrq[sbdrq_head].stat == SBDR_STATUS_ENQUEUED) {
    process_request();
...
/**
 * @return true on a error or failure to flush output
 */
bool send_packet() {
}
 ...
 * For serusb, the request was the string to be sent to the device
 * For CANable, I have a complete structure including the CAN message to be sent
 */
static void enqueue_sbreq(int rcvid, can_msg_t *can_msg, uint8_t *rep_buf, int buflen) {
  //(int type, int rcvid, char *req, unsigned short n_reads ) {
  can_request_t *sbr = &sbdrq[sbdrq_tail];
  int i;
  int new_tail = sbdrq_tail+1;
  int old_tail;
...
static int sb_data_arm(void) {
  int cond;
  cond = ionotify( can_fd, _NOTIFY_ACTION_POLLARM,
...
/**
 ...
 @param sp pointer to pointer, updated on return
 @param arg pointer to uint16_t where the converted value is stored
...
static void sb_read_usb(void) {
  do {
    ...
    nb = read(can_fd, &sb_ibuf[sb_ibuf_idx], nbr);
    if ( nb < 0 ) {
...
static void init_sercan(dispatch_t *dpp, int ionotify_pulse,
                          int timeout_pulse) {
  struct sigevent timeout_event;
  can_fd = open(serusb_port, O_RDWR | O_NONBLOCK);
  if (can_fd == -1)
    nl_error(3,"Error opening USB subbus: %s", strerror(errno));
  serial_setup(can_fd, serusb_baud_rate, 8, 'n', 1, 1, 0);
  /* flush anything in the input buffer */
  { int n;
    char tbuf[256];
    do {
      n = read(can_fd, tbuf, 256);
      if ( n == -1) {
...
void incoming_sbreq( int rcvid, subbusd_req_t *req ) {
  ...
  switch ( req->sbhdr.command ) {
    case SBC_READACK:
      setup_rw_request(rcvid, CAN_CMD_CODE_RD, req->data.d1.data, 0);
      break;
    case SBC_MREAD:
      setup_mread_request(rcvid, req->data.d4.multread_cmd, req->data.d4.n_reads);
      return;
    case SBC_WRITEACK:
      setup_rw_request(rcvid, CAN_CMD_CODE_WR_INC, req->data.d0.address, req->data.d0.data);
...
void CAN_issue_init() {
  ...
  enqueue_sbreq( SBDR_TYPE_INTERNAL, 0, "C\rS2\rO\rV\r", 0 );
  // write(can_fd, "C\rS2\rO\rV\r", 9);
...
void init_subbus(dispatch_t *dpp ) {
  ...
  init_sercan(dpp, ionotify_pulse, timer_pulse);
